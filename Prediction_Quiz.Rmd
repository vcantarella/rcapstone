...
<li> \$166,721.30 </li>
<li> \$172,994.50
<li> \$177,220.30 <- This One!
</ol>
```{r Q1}
# type your code for Question 1 here, and Knit
mean(exp(predict(model.AIC, newdata = ames_test)))
```
...
<li> When predicting to `ames_train`, the RMSE for `model.AIC` is higher than the RMSE for `model.full`.  However, when predicting to `ames_test`, the RMSE for `model.full` is higher. <- This one is the answer.
...
rmse.AIC.train <- sqrt(mean((ames_train$price - exp(predict(model.AIC, ames_train)))^2))
rmse.full.train <- sqrt(mean((ames_train$price - exp(predict(model.full, ames_train)))^2))
...
Because the model is generally built to fit the dataset, the fit of out-of-sample-data is probably worse.
...
pred_rmse <- c(0,0,0)
j = 1
for (i in c("HPM", "BPM", "BMA")){
  pred.test <- predict(model.bas, newdata = ames_test, estimator= i)
pred.rmse <- sqrt(mean((exp(pred.test$fit) - ames_test$price)^2))
pred_rmse[j] = pred.rmse
j= j+1
}
pred_rmse
```
...
<ol>
<li> True
<li> False <- This One. THe model has a tendency to under predict expensive houses
</ol>
