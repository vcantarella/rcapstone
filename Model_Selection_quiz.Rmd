...
<li> `Bedroom.AbvGr` </li> 
<li> `Overall.Qual` </li>
<li> `Land.Slope` </li> <- This one!
</ol>
```{r Q1}
# type your code for Question 1 here, and Knit

price_log_lm <- lm(log(price)~log(area)+log(Lot.Area)+Bedroom.AbvGr+Overall.Qual+Land.Slope,
                   data = ames_train)
summary(price_log_lm)

library(MASS)
stepAIC(price_log_lm, k = 2)
stepAIC(price_log_lm, k = log(1000))
```
...
<li> Larger houses on average have more bedrooms and sell for higher prices.  However, holding constant the size of a house, the number of bedrooms decreases property valuation. <- This one!
...
price_bed <- lm(log(price)~Bedroom.AbvGr, data = ames_train)
price_area <- lm(log(price)~log(area)+Bedroom.AbvGr, data = ames_train)
library(broom)
tidy(price_bed)
tidy(price_area)
```
...
In this question there is also the fact that area is a conofounder for the number of bedrooms. As normally houses with more area have more bedrooms.
...
price_area2 <- lm(log(price)~log(area), data = ames_train)
ames_train%>%
  mutate(residual = resid(price_area2))%>%
  group_by(Neighborhood)%>%
  summarise(avg_resid = mean(residual))%>%
...
summary(price_area2)
ames_train%>%
  mutate(fittede = exp(fitted(price_area2)))%>%
  mutate(error = sqrt((price - fittede)^2))%>%
  group_by(Neighborhood)%>%
...
qual_lm <- lm(log(price)~Overall.Qual+Bsmt.Qual+Garage.Qual, data = ames_train)

dim(ames_train)[1] - length(fitted(qual_lm))
...
<li> Recode all `NA` values as a separate category, since houses without basements or garages are fundamentally different than houses with both basements and garages. <-RECODE!!
</ol>
...
<ol>
<li> 075: 2-1/2 story houses <- This One!!
...
ov_model <- lm(log(price)~Overall.Cond+Overall.Qual, data = ames_train)
ames_train%>%
  mutate(fitted = exp(fitted(ov_model)))%>%
  group_by(MS.SubClass)%>%
  summarise(median_pred_price = median(fitted))%>%
  arrange(desc(median_pred_price))
```
...
as_tibble(hatvalues(ov_model))[c(125,268,640,832),]
```
...
<ol>
<li> Holding constant all other variables in the dataset, on average, an additional bedroom will increase housing price by $k$ percent. <- This ONe!
...
<ol>
<li> True, restricting the model to only include observations with normal sale conditions increases the $R^2$ from 0.547 to 0.575. <- TRUE, THis oNe!
...
ames_normal <- ames_train%>%
  filter(Sale.Condition == "Normal")
price_area_normal <- lm(log(price)~log(area), data = ames_normal)
summary(price_area_normal)
```
